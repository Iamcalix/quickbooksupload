...
console.log("--- Testing NMB Parsing ---");
const nmbResult = parseTransactions(nmbSample, 'NMB');
const nmbTx = nmbResult.successful[0];
console.log(`Extracted userId (Expect 22410063786): ${nmbTx?.userId}`);
console.log(`IsValid: ${nmbTx?.isValid}`);

console.log("\n--- Testing CRDB Parsing ---");
const crdbResult = parseTransactions(crdbSample, 'CRDB');
const crdbTx = crdbResult.successful[0];
console.log(`Extracted refId (Expect 19bdb0f42ad57818): ${crdbTx?.refId}`);
console.log(`IsValid: ${crdbTx?.isValid}`);
